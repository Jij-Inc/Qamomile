...
- [Cell format (jupytext percent)](#cell-format-jupytext-percent)
- [Tags frontmatter](#tags-frontmatter)
- [Topic-tag vocabulary](#topic-tag-vocabulary)
- [Opening cells](#opening-cells)
...
Use this block verbatim (`jupytext_version: '1.19.1'`, no `formats:` line). `jupytext` is forgiving about the version string, but copy the canonical block rather than inventing variants — the build pipeline expects this exact shape.

## Cell format (jupytext percent)

The `.py` file is a notebook in jupytext **percent** format. Two cell separators matter, and `jupytext --to ipynb tutorial.py` must produce a clean notebook from them:

```python
# %% [markdown]
# A markdown cell. Every line is prefixed with `# ` (hash + space).
# Inline math $f(x)$ and block math both work:
#
# $$ \mathcal{L}(\theta) = \tfrac{1}{N} \sum_i \ell_i $$

# %%
# A code cell. Plain Python, no prefix.
import numpy as np
```

Mechanics that bite if you get them wrong:

- **The first `# %%` (or `# %% [markdown]`) starts cell 1.** Everything above it is the file header (the YAML block).
- **A blank line inside a markdown cell ends the cell.** For vertical spacing inside the cell, use a bare `#`, never a fully blank line. (See "Markdown and heading conventions" below for the full `# `-prefix rule.)
- **Do not put `# %%` inside a string or docstring.** jupytext is regex-based and will split the file on it.
- **`# comment` lines inside a code cell stay Python comments** — they do not render as markdown. To get prose, you need a `# %% [markdown]` separator.

The complete reference example in [assets/example_tutorial.py](../assets/example_tutorial.py) is a working file in this exact format — open it when a separator placement is unclear.
...
# # <Tutorial title>
#
# <Abstract paragraph(s) go here — no list, see "Opening cells" below.>
```
...
2. **Install-stub code cell.** A code cell holding only the install stub, exactly as other tutorials have it:
...
3. **Imports code cell.** A *separate* code cell immediately after the install stub, holding every `import` the tutorial uses. Imports are *not* scattered next to the code cells that first use them — readers should see the full dependency footprint of the article in one place, at the top. Keep the install stub and the imports in two distinct cells; do not merge them.
...
- These are in the dev environment used by `jupyter-book build .`, so existing tutorials (e.g. `pce_maxcut.py`) import them freely. Use them where a plot or a graph layout teaches something the numbers cannot.
...
When a tutorial in `integration/` uses an optional backend, the abstract cell must tell the reader which extra to install. For tutorials in the other three categories, **do not** import optional backends — a `tutorial/`, `algorithm/`, or `usage/` page must run after a bare `pip install qamomile` plus the dev deps, so use `qamomile.qiskit.QiskitTranspiler` as the default backend (it ships with the core install). Do not reach for an optional backend just to make an example prettier.
...
This is the pattern used in `docs/en/algorithm/qe_mcmc.py` and `docs/en/algorithm/pce_maxcut.py`. Copy that shape; do not invent another knob.
...
- **Notes and asides use MyST callouts, not `>` blockquotes.** Wrap the note in a `:::{note}` … `:::` fence (one `# ` per line in the percent source), and lead with a short bold topic label:

  ```markdown
  # :::{note}
  # **Gate convention.** Qamomile's rotation gates carry a $1/2$ factor …
  # :::
  ```

  `:::{warning}` / `:::{tip}` are available for the matching admonition types. The `>` blockquote form is not used for notes.
- **Cross-reference a section** by labeling its heading with `(label)=` on the line directly above it, then linking with the empty-text form `[](#label)` (mystmd fills in the heading title). The full rule and rationale live in `references/section_guide.md` → "Cell and cross-reference conventions".
- Heading hierarchy is **strict**:
  - H1 (`# <title>`) — the tutorial title, once, inside the Abstract cell.
  - H2 — exactly these five labels, in order, and nothing else: `## Background`, `## Algorithm`, `## Implementation`, `## Result`, `## Summary`. The Abstract has no H2 heading.
...
The docs are built with the MyST engine (`myst.yml`), which resolves DOI-keyed citations natively. **Cite an academic paper with the `{cite:p}` role keyed by the paper's DOI**, exactly as the existing tutorials do (see [docs/en/algorithm/qsci.py](../../../docs/en/algorithm/qsci.py)):

```markdown
# QAOA was introduced by Farhi et al. {cite:p}`10.48550/arXiv.1411.4028`
# as a hybrid quantum-classical heuristic for combinatorial optimization.
```

MyST resolves the DOI metadata and renders a numbered citation plus an auto-generated reference list, so no `references.bib` is needed.

Notes:

- The cite key is the **bare DOI** (`10.48550/arXiv.<id>` for arXiv preprints), not a URL. Do **not** write a DOI URL as a bare link or wrap it in `(...)` / `[text](url)`: a bare `(https://doi.org/...)` in body text renders as a plain, awkward link rather than a citation. Do not hand-type "(Farhi et al. 2014)", and do not paste raw `arxiv.org/abs/...` URLs.
- One `{cite:p}` per cited claim, placed right after the author names or the claim it supports, like the example above.
- Where to cite: primarily in `## Background` (when the problem itself comes from a paper) and `## Algorithm` (for the original method paper, encoding schemes, or compression-rate results). Do **not** sprinkle citations through `## Implementation` — that section is about the Qamomile API, not the literature. The `## Summary` section does not carry citations.
- **Plain markdown links remain correct** for non-academic references — blog posts, GitHub repos, Qamomile docs, vendor documentation. The `{cite:p}` rule applies only to DOI-backed papers.
