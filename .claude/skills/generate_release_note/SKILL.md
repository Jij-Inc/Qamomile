...
**ユーザー視点で「このリリースで何ができるようになったか」だけを書く**。普通のユーザーが直接触らない情報は概要には入れない — それぞれのサブセクションで詳述する。

- 公開API・公開機能のうち、ユーザーが実際に呼ぶものだけを名指しする
- **羅列を避ける**: 1機能あたり「動作する API 名 1 つ + 代表例 1〜2 個」まで。`q[1::2]`, `q[lo:hi]`, ネスト view, スライス境界の複合ゲート, ... のような機能カタログは出さない
- **内部実装語を出さない**: IR primitive 名、コンパイラパス名、新規例外クラスの階層、アフィン型、サポート対象 SDK の列挙などは概要から外す
- **ユーザーが直接触らない内部更新は1文に圧縮する**: 「`@qkernel` を外部 DSL のサブグラフとして扱う基盤となる IR の内部的な更新もあります」程度。canonical form, content hash, param_slots, JSON/msgpack シリアライズ ... のような部品名は本文 (`## Internal Changes`) でだけ展開する
- 結果として概要は **3〜5 文 / 5〜8 行** に収まるはず。これより長くなったら羅列・実装語の混入を疑う

**Why**: 概要を読むのは「このリリースで自分のコードに何が起きるか」を3秒で知りたい人。詳細を知りたい人は本文を読む。概要に部品名を並べると、内部更新だけのリリースに見えて、目玉機能が埋もれる。

#### 共通の言い回しルール

リリースノート本文全体に適用される細かいルール。

- **クラス間の関係は親への subclass 関係だけ書く**: 「`Foo` is a sibling of `Bar`」「`Foo`は`Bar`の兄弟例外」のように同階層クラスの兄弟関係は書かない。`subclass of AffineTypeError` /「`AffineTypeError`のサブクラス」と親クラスを述べるだけで十分。「兄弟例外」は日本語として不自然
- **「意図的に」「deliberately」を使わない**: 未サポート機能を述べる時に「意図的にサポートしません」「deliberately not supported」と書かない。単に「サポートしていません」「not supported」で良い。なぜ意図的かを語らないと意味が変わる場面はほぼ無い
- **未公開の内部議論・社内ドキュメントへの参照を入れない**: 「(2026-05-16のIR設計議論)」「(internal RFC #42)」のような、公開リポジトリ外のアーティファクトへの言及はしない。リリースノートは **「前バージョンタグから最新mainまでのdiff」と公開された PR / issue / コード** だけから書く。`**Why**:` で動機を書く時も、コード上の事実(このカーネルがこの IR ノードを必要としているから等)に基づく
- **「sibling」「twin」「umbrella」など英語の比喩を直訳しない**: 関係を技術的にそのまま述べる(「親クラス」「同種の」「まとめて」など)
...
### Phase 5b: 説明文の技術主張をコードと照合(必須)

スニペット実行で拾えるのは「動く / 動かない」だけ。pass 名・クラス名・件数・サブクラス関係・raise する例外型・サポートされる引数・前提条件などの**説明文中の事実主張**は別途コードと突合する必要がある。

**PR 説明の prose をそのまま引用しない**:

- PR 本文の文章には、レビュー過程で **rename / 削除された pass・クラス名**がそのまま残っていることがある(例: `SliceLinearityCheckPass` は最終的に `SliceBorrowCheckPass` になっていた)
- 「N 個の raise サイト」「3 つの新規パス」のような **件数**も書き換え後とずれている可能性がある
- PR 本文の表現はあくまでドラフトとして扱い、リリースノートに書く前に対応するコード(クラス定義 / `__all__` / `__init__.py` / 実際の `raise` 行)を grep して確認する

**主な照合対象**:

| 主張の種類 | 照合先 |
|---|---|
| クラス名・関数名・モジュールパス | `qamomile/` 配下に同名の定義があるか `grep`/`rg` |
| 公開 API パス (`qamomile.foo.bar`) | `qamomile/foo/__init__.py` の `__all__` / re-export |
| サブクラス関係 (`X is a subclass of Y`) | `class X(Y):` を直接読む |
| raise する例外型 | 主張した箇所付近で `raise <ExceptionName>` を grep |
| 件数 (「N 個の raise サイト」) | `grep -c` で実測 |
| パイプライン順序・ステージ | `Transpiler.transpile()` の本体を読む |
| 例の `print(...)` 期待出力 | 実際に1回走らせて output を貼る(Phase 5 と兼ねる) |

**codex スキルでの自動照合(推奨)**:

書き上がった md の各サブセクションを codex に渡し、「この主張をコードと比較して報告」を依頼する。報告フォーマット: `file:line + 主張 + コードが示すもの + severity (P0/P1/P2/P3)`。

```bash
codex exec --skip-git-repo-check --sandbox read-only --color never \
  "docs/en/release_notes/v<X_Y_Z>.md の技術的主張をコードと突き合わせて検証して..." \
  < /dev/null
```

**繰り返し**: 指摘が出たら本文を直し、`codex exec resume --last --skip-git-repo-check "適用したので再検証" < /dev/null` で再実行。**指摘 0 になるまで**ループする(P3 は判断、P2 以上は直す)。

> **注意**: `codex exec resume` は `--color` フラグを受け付けない(`codex exec` だけが受け付ける)。resume では `--color` を外す。
