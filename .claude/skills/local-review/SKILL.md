...
- Loops inside a qkernel body over quantum operations MUST use Qamomile's symbolic `range` via whichever `qamomile.circuit` alias is imported — `qm.range(...)` or `qmc.range(...)` are both accepted (project convention; the AST transformer also happens to accept plain `range(...)`, but the convention is to make quantum-side iteration explicit). Dict iteration can use either `for k, v in mapping.items()` (native) or `for k, v in qm.items(mapping)` / `for k, v in qmc.items(mapping)` (Qamomile form, as in `qamomile/circuit/algorithm/fqaoa.py:115,118`). Plain Python iteration over arbitrary iterables (`for x in some_list:`) is **not** captured as an IR `ForOperation` — restrict that form to iteration over precomputed concrete data used via closures (e.g., numpy arrays referenced from within the kernel body).
