...
7. **Python style** (Section G) — Google-style docstrings on **all** functions, methods, and classes (public and private alike, with `Args`/`Returns`/`Raises` as applicable and type hints)? Modern Python syntax? No dead code?
