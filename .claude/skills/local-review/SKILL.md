...
- **Jupytext percent-format `.py` is the source of truth.** Every tutorial `.py` must have a committed `.ipynb` that (a) exists, (b) stays in sync when its `.py` changes via `jupytext --sync`. Any of these failing is **P1**. The `.ipynb` execution_count and outputs are intentionally cleared (set to `null` / `[]`) so the repo doesn't accumulate stale output state; CI tests execute the notebooks separately and validate the output. An `.ipynb`-only change (no corresponding `.py` update) is **P2** — it bypasses the source-of-truth.
