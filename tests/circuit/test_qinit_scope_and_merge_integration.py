...
Bug #6: IfOperation merge slots — merge outputs were never allocated or emitted,
    causing merge output UUIDs to be missing from qubit_map.  Fixed in
    emit_support/resource_allocator.py (ResourceAllocator), standard_emit.py (_register_merge_outputs,
...
Unit tests for the individual passes live in ``tests/transpiler/``:
    ...
    - ``test_emit_support.py``   (merge output allocation)
