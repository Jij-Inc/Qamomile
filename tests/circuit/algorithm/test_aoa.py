...
@qmc.qkernel
def _wrap_aoa_state_basis_state_expval(
    p: qmc.UInt,
    quad: qmc.Dict[qmc.Tuple[qmc.UInt, qmc.UInt], qmc.Float],
    linear: qmc.Dict[qmc.UInt, qmc.Float],
    n: qmc.UInt,
...
@pytest.mark.parametrize("name,TranspilerCls", BACKENDS)
@pytest.mark.parametrize("n,k", [(2, 0), (2, 1), (3, 1), (3, 2), (4, 2)])
def test_aoa_state_basis_state_expval_z_sum_matches_analytic(name, TranspilerCls, n, k):
    """Tests that aoa_state_basis_state with zero angles gives <sum Z_i> = n - 2k.

    With gammas=[0] and betas=[0] the circuit is the deterministic basis state
    with the last k qubits set to |1>.  Each |0> qubit contributes +1 and each
    |1> qubit contributes -1, so <sum_i Z_i> = (n-k) - k = n - 2k exactly.
    """
    ...
    transpiler = TranspilerCls()
    exe = transpiler.transpile(
        _wrap_aoa_state_basis_state_expval,
        bindings={
            "p": 1,
            "quad": {},
            "linear": {},
...
@qmc.qkernel
def _wrap_hubo_aoa_state_dicke_expval(
    p: qmc.UInt,
    quad: qmc.Dict[qmc.Tuple[qmc.UInt, qmc.UInt], qmc.Float],
    linear: qmc.Dict[qmc.UInt, qmc.Float],
    higher: qmc.Dict[qmc.Vector[qmc.UInt], qmc.Float],
...
@qmc.qkernel
def _wrap_hubo_aoa_state_basis_state_expval(
    p: qmc.UInt,
    quad: qmc.Dict[qmc.Tuple[qmc.UInt, qmc.UInt], qmc.Float],
    linear: qmc.Dict[qmc.UInt, qmc.Float],
    higher: qmc.Dict[qmc.Vector[qmc.UInt], qmc.Float],
...
@pytest.mark.parametrize("name,TranspilerCls", BACKENDS)
def test_hubo_aoa_state_superposition_sample_returns_valid_bitstrings(
    name, TranspilerCls
):
...
def test_hubo_aoa_state_dicke_expval_z_sum_matches_analytic(name, TranspilerCls, n, k):
    """Tests that hubo_aoa_state_dicke with zero angles gives <sum Z_i> = n - 2k.

    With gammas=[0] and betas=[0] the circuit is the bare Dicke state |D^n_k>.
    ...
    transpiler = TranspilerCls()
    exe = transpiler.transpile(
        _wrap_hubo_aoa_state_dicke_expval,
        bindings={
            "p": 1,
            "quad": {},
            "linear": {},
...
@pytest.mark.parametrize("name,TranspilerCls", BACKENDS)
@pytest.mark.parametrize("n,k", [(2, 0), (2, 1), (3, 1), (3, 2), (4, 2)])
def test_hubo_aoa_state_basis_state_expval_z_sum_matches_analytic(
    name, TranspilerCls, n, k
):
    """Tests that hubo_aoa_state_basis_state with zero angles gives <sum Z_i> = n - 2k.
    ...
    transpiler = TranspilerCls()
    exe = transpiler.transpile(
        _wrap_hubo_aoa_state_basis_state_expval,
        bindings={
            "p": 1,
            "quad": {},
            "linear": {},
...
def test_hubo_aoa_state_dicke_sample_preserves_hamming_weight(
    name, TranspilerCls, n, k
):
...
@pytest.mark.parametrize("name,TranspilerCls", BACKENDS)
@pytest.mark.parametrize("n,k", [(3, 1), (4, 2)])
def test_hubo_aoa_state_basis_state_sample_preserves_hamming_weight(
    name, TranspilerCls, n, k
):
