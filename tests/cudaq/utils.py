...
class CudaqUtils:
    ...
    QIR_OPERATIONS: Final[list[str]] = (
        list(QIR_PARAMETRIC_OPERATIONS_PARAM_MAPS.keys())
        + QIR_NON_PARAMETRIC_OPERATIONS
    )
            ...
            for block in func.basic_blocks:
                for inst in block.instructions:
                    if isinstance(inst, pyqir.Call):
                        ...
                        if (
                            callee_name
                            in CudaqUtils.QIR_PARAMETRIC_OPERATIONS_PARAM_MAPS
                        ):
                            total_params += (
                                CudaqUtils.QIR_PARAMETRIC_OPERATIONS_PARAM_MAPS[
                                    callee_name
                                ]
                            )
        return total_params
        ...
        for func in module.functions:
            for block in func.basic_blocks:
                for inst in block.instructions:
                    if isinstance(inst, pyqir.Call):
                        callee_name = inst.callee.name
                        if callee_name in CudaqUtils.QIR_OPERATIONS:
                            try:
