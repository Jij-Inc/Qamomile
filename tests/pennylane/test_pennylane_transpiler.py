...
def test_transpile_hamiltonian_with_multiple_terms(transpiler):
    ...
    assert all(
        isinstance(op, (qml.PauliY, qml.PauliZ, qml.PauliX)) for op in ops_second_term
    )
...
def test_apply_parametric_single_qubit_gate_unsupported(transpiler):
    ...
    with pytest.raises(
        NotImplementedError, match="Unsupported parametric single-qubit gate"
    ):
        transpiler._apply_parametric_single_qubit_gate(
            MockParamGate(), params={"theta": np.pi}
        )
...
def test_apply_parametric_two_qubit_gate_unsupported(transpiler):
    ...
    with pytest.raises(
        NotImplementedError, match="Unsupported parametric two-qubit gate"
    ):
        transpiler._apply_parametric_two_qubit_gate(
            MockParamGate(), params={"theta": np.pi}
        )
