...
def test_scs_schedule_for_weight_two_builds_one_pair_and_one_triplet():
	...
	np.testing.assert_array_equal(triplets_indices, np.asarray([[1, 2, 3]], dtype=np.uint32))
...
def test_dicke_state_composition_schedule_repeats_local_schedule_per_block():
	...
	expected_triplets_indices = np.vstack([local_triplets, local_triplets + 3]).astype(np.uint32)
...
@pytest.mark.parametrize(
	...
	[
		...
		({"n_qubits": 5, "block_size": 0, "hamming_weight": 1}, "block_size must be > 0"),
		({"n_qubits": 5, "block_size": 2, "hamming_weight": 1}, "divisible by block_size"),
		({"n_qubits": 6, "block_size": 3, "hamming_weight": 4}, "Require 0 <= hamming_weight <= block_size"),
	],
)
