...
def test_engines_do_not_import_circuit_frontend() -> None:
    """Engine packages depend on public circuit APIs, never frontend internals."""
    violations: list[str] = []
    for engine in ENGINE_PACKAGES:
        for reference in _iter_import_references(QAMOMILE_ROOT / engine):
            if any(
                ...
                violations.append(
                    _format_violation(
                        reference,
                        "engine imports qamomile.circuit.frontend",
                    )
                )

    assert not violations, (
        "Engine packages must not depend on circuit.frontend internals. "
