...
- **Tuple payloads**: Python tuples in payload positions are wrapped as `{"$tuple": [...]}` so the tuple-vs-list distinction survives the round-trip. This keeps `DictRuntimeMetadata.bound_data` keys hashable after decode (`DictValue.get_bound_data()`) and keeps `content_hash` stable across a dump/load cycle (frozen-metadata `repr` feeds the hash). Plain payload dicts are validated at encode time: non-`str` keys and the reserved wrapper keys raise `TypeError` instead of silently corrupting.
- **Container I/O values**: `Block.input_values` / `output_values` / operand and result lists may reference `TupleValue` / `DictValue` (e.g. a kernel taking a `Dict` argument); the decoder accepts container values in those positions.
- **Nested-block value tables are local**: operation-owned nested blocks (`ControlledUOperation.block`, `CompositeGateOperation.implementation_block`, `InverseBlockOperation` blocks) each carry a self-contained `value_table` with exactly the Values that nested block references — the parent's table is never duplicated into them, keeping the wire size linear in IR size. The decoder materializes each block dict against its own table, so both the old (duplicating) and new (local) layouts decode identically.
...
- **Executing deserialized IR**: `Transpiler.transpile_block(block, bindings=..., parameters=...)` compiles a loaded `AFFINE` / `ANALYZED` Block into an `ExecutableProgram` without the originating `QKernel` — the entry point for embedding Qamomile programs as data inside an external orchestrator (serialize on the client, `load_json` / `load_msgpack` + `transpile_block` + execute on the runner). Compile-time values folded at trace time are restored from `Block.param_slots` (re-binding a `COMPILE_TIME_BOUND` slot raises); array `bindings` supplied at this stage resolve symbolic `Vector` shape dims, so an unbound kernel can be serialized once and sized by the receiver. Scalar `Observable` arguments remain runtime-supplied: pass the `Hamiltonian` via `bindings` to `transpile_block` (the standalone `$hamiltonian` wire helpers `hamiltonian_to_dict` / `dict_to_hamiltonian` are exported for hosts that ship observables next to the Block).
