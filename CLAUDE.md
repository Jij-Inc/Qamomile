...
- **No `@`-mentions.** Never write bare `@username` / `@org/team` tokens in GitHub-tracked text — they trigger notifications. In prose refer to decorators descriptively ("the qkernel decorator"); show a literal `@…` only inside a fenced code block or inline code span.
