...
def emit_self_call_forward_ref(
    ...
    Args:
        ...
        inputs_map (dict[str, ValueLike]): Actual argument values keyed by
            parameter name.
    ...
    Raises:
        FrontendTransformError: If an unmatched array or structural output
            cannot be synthesized for the forward reference.
    """
    ...
    for i, out_type in enumerate(kernel.output_types):
        ...
        if is_tuple_type(out_type) or is_dict_type(out_type):
            output_kind = "tuple" if is_tuple_type(out_type) else "dict"
            raise FrontendTransformError(
                f"Self-recursive @qkernel '{kernel.name}' has a {output_kind} "
                f"output at position {i} with no matching input of the same "
                "type. Forward-reference emission cannot synthesize its "
                "element graph; add a matching structural input or remove "
                "the self-recursion."
            )
