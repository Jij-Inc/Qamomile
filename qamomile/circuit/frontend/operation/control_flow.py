...
def _create_merge_for_values(
    ...
    # A quantum merge over two SSA versions of the same logical wire still
    # denotes that wire. Preserve its logical identity so operation-owned
    # unitary validation (for example SELECT cases) and downstream wire
    # analyses do not mistake a conditional gate update for a fresh qubit.
    # When branches really return different wires, keep a fresh logical ID to
    # represent the conditional choice and let discard validation reject it
    # where required.
    preserved_logical_id = (
        true_v.logical_id
        if true_v.type.is_quantum() and true_v.logical_id == false_v.logical_id
        else None
    )
    ...
    if preserved_logical_id is not None:
        merge_output = dataclasses.replace(
            merge_output,
            logical_id=preserved_logical_id,
        )
