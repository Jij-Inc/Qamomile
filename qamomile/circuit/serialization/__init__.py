...
graph-local identities. Fixed ``ResourceEstimate`` costs attached to opaque
callables are preserved, while context-dependent Python cost callbacks are
rejected because they remain process-local. Invocation bindings, runtime values,
prepared compiler modules, backend artifacts, standalone resource-estimation
outputs, and Python evaluation performed during tracing are outside the format.
