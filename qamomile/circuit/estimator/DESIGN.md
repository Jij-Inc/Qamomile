...
| **ForItemsOperation** | パラレル推定では sequential_depth × `\|dict_name\|` シンボル。具体シミュレーション中は body を1回だけ実行（辞書サイズ不明のため） |
...
3. **既知型（QFT/IQFT のみ）**: 公式を使用（下記）
4. **エラー**: いずれもない場合は ValueError

注意: gate_counter.py の ResourceMetadata 抽出（§4.3）は型付きフィールド（`meta.single_qubit_gates` 等）を使用するのに対し、depth_estimator.py は `meta.custom_metadata` 辞書を使用する。この非対称性に注意が必要。
