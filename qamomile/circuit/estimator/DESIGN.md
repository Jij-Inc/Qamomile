...
- **`QInitOperation`**:
  ...
  - 結果が非量子型 → 0
- **入力 qubit（`Block.input_values`）**: トップレベル呼び出し（`qubits_counter(block)`）時にのみカウント。再帰的な内部呼び出し（`_count_from_operations`）ではスキップし、二重カウントを防ぐ。

### 3.2 クリーンコール判定

`_is_clean_call(block)` は Block が「クリーンコール」であるかを判定する。

**判定条件**: Block の全ての量子型 `return_values` の `logical_id` が `input_values` の `logical_id` 集合に含まれていること。
...
3. 本体の内部カウントを再帰的に算出
4. **ループ変数に依存する場合**: 内部カウントの `total`, `two_qubit`, `multi_qubit`, `oracle_calls`, `oracle_queries` のいずれかの free_symbols にループ変数が含まれる → 変数変換 `loop_var = start + step * k` を適用し `Sum(inner_count.subs(loop_var, start + step*k), (k, 0, iterations-1))` 式。`iterations = Max(0, ceiling((stop - start) / step))` で Python `range()` セマンティクスと一致。
5. **ループ変数に依存しない場合**: `inner_count × Max(0, ceiling((stop - start) / step))`
...
2. **Implementation**: Block の分解回路を再帰的にカウント（仮引数→実引数のマッピング適用）
...
`num_targets` の決定: `controlled_block` が Block の場合は量子型 `input_values` の数。それ以外は `target_operands` の長さ（なければ 1）。
...
| `symbolic_iterations(start, stop, step)` | `Max(0, ceiling((stop − start) / step))` としてシンボリック反復回数を算出（Python `range()` セマンティクスと一致） |

## qubits_counter.py

| 関数 | 概要 |
|---|---|
| `qubits_counter(block)` | エントリポイント。Block/list[Operation] の qubit 数を返す。Block の場合は input_values + operations 両方をカウント |
...
| `_count_input_qubits(input_values, resolver)` | Block.input_values の qubit 型をカウント。トップレベルのみ使用 |
| `_is_clean_call(block)` | Block がクリーンコール（入力 qubit のみ返す）かを判定 |
...
| 関数 | 概要 |
|---|---|
| `count_gates(block)` | エントリポイント。Block/list[Operation] のゲート数を返す |
