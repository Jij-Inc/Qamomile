...
def _emit_quri_composite_operation(
    ...
    _map_operand_result_groups(
        [result for result in op.results if result.type.is_quantum()],
        [*control_index_groups, *target_index_groups],
        qubit_map,
    )
...
def _emit_quri_nested_controlled_u(
    ...
    with bracket_zero_controls(emit_pass, circuit, resolved.zero_control_phys):
