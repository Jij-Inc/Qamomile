...
#
# **目標:** QamomileでQAOAソルバーを構築し、[OMMX Quantum Benchmarks](https://github.com/Jij-Inc/OmmxQuantumBenchmarks)データセットからロードした**Low Autocorrelation Binary Sequences (LABS)**インスタンスに対して実行し、[`ommx-pyscipopt-adapter`](https://github.com/Jij-Inc/ommx-pyscipopt-adapter)経由で利用できる古典ソルバーSCIPと結果を比較します。QAOA側とSCIP側はどちらも*同じ*`ommx.v1.Instance`を入力とするため、主な違いはアルゴリズムそのものです。そのため直接比較できます。
...
#
# **OMMX**([Open Mathematical prograMming eXchange](https://jij-inc.github.io/ommx/en/introduction.html))は、数理最適化問題をツール間で受け渡すためのデータフォーマットです。`ommx.v1.Instance`には目的関数、制約条件、決定変数のメタデータ、必要に応じて参照解が格納されます。
#
...
#
# 各ベンチマークインスタンスは`ommx.v1.Instance`として表現されるため、`QAOAConverter`を含む`ommx.v1.Instance`対応のQamomileワークフローは、追加のアダプタコードなしにこれらの問題を扱えます。同じ`Instance`は`ommx-pyscipopt-adapter`のような古典側のOMMXアダプタにも渡せるので、1つの問題定義を量子・古典の両方のワークフローで使えます。
...
# ### 最終サンプリング
#
# 最適化されたパラメータでショット数を増やして再度サンプリングします。その後、元の`ommx.v1.Instance`に対してデコードすることで、返ってくる`ommx.v1.SampleSet`が元のQUBO目的値を直接報告します。このQUBO定式化では、補助変数$z$が積$x_i x_{i+k+1}$を正しく表現しているサンプルではペナルティが発生せず、目的値がLABSエネルギー$E(\boldsymbol{s}) = \sum_k c_k^2$と一致します。一方、$z = x_i x_{i+k+1}$という暗黙の関係を破ったサンプルには、インスタンスに組み込まれたプレースホルダー$P$に比例する加算ペナルティが課されます。
...
# ### 目的値の分布
#
# QAOAは一つの解ではなくビット列の分布を返します。下のヒストグラムは、最適化済みパラメータで取得した各ショットのQUBO目的値です。赤の点線は参照最適値$E^\star$を示します。その線上、またはすぐ右側にあるサンプルは、$x$の値がLABS和を最小化し、$z$の値が積を正しく符号化しています。線から大きく右に離れたサンプルは、$z$の不整合に対するペナルティを払っています。
...
# ## 結果の比較
#
# SCIPは最適解を決定論的に返します。一方、QAOAはビット列の*分布*を返します。そこでQAOA側は**ベストショット**(全サンプル中で最も低い目的値を達成したビット列)と、参照最適値に対する**ヒット率**(その値を達成したショットの割合)を見てみます。
...
#
# 経過時間の列は比較の*形*を示すものです。SCIPはCPU上でQUBOを直接解くのに対し、QAOA側の計測値はステートベクタシミュレータ上での古典・量子最適化ループ全体を含みます。$n$が大きくなると、厳密な分枝限定法と、多項式深さのヒューリスティック回路との間に*定性的*なトレードオフが生じます。OMMX Quantum Benchmarksのようなデータセットは、この両面を評価するために有用です。
