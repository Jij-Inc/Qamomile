...
#
# レシピは[MaxCutに対するQAOAチュートリアル](../algorithm/qaoa_maxcut.ipynb)と同じです。一様重ね合わせから始め、 $p$ 回ぶんのコスト層+ミキサー層を適用し、最後に測定を行います。Qamomileの回転ゲートは $e^{-i\theta/2}$ という規約に従うため、 $1/2$ 係数の扱いはコスト層とミキサー層で異なります。ミキサー側では `rx` に $2\beta$ を渡すことで $1/2$ がちょうど打ち消され、教科書通りの $e^{-i\beta X}$ が得られます。一方コスト側では `rzz` に $J_{ij} \cdot \gamma$ を渡しており、 $1/2$ は**打ち消されません**。代わりに、足りない係数 $2$ は変分パラメータ $\gamma$ そのものに吸収されています(したがってここで使う $\gamma$ は、教科書のQAOA $\gamma$ の半分に相当します)。
