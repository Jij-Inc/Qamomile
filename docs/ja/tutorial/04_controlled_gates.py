...
#
# 本セクションの形は`num_controls`が`qmc.UInt`ハンドル(または`n - 1`のような`UInt`式)であることを要求します。共通する性質は2つです。
#
# - 呼び出し側の制御引数は、`Vector[Qubit]`または`VectorView`の単一poolです。複数のpositionalな`Qubit`引数ではありません。
# - *active*な制御の数は`qmc.control(..., num_controls=...)`の評価時ではなく、`bindings`からtranspile時に決まります。
#
# `controlled_indices=`keywordはsymbolic mode専用で、poolのどのスロットがactiveな制御として実際に配線されるかを指定します(残りはそのまま素通りします)。
...
# %% [markdown]
# ### 6.8 同じpoolスロットをtargetに再利用 (symbolic)
#
# symbolic modeでは制御引数は*pool全体*で、inactiveなスロット(`controlled_indices`に含まれていないスロット)は図上pass-throughのwireとして残ります。同じpool内のinactiveなスロットを取り出してtargetとして渡したくなることがあります。例えば`cg(pool, pool[2], controlled_indices=[0, 1, 3])`として`pool[2]`をcontrolled-Uのtargetにする、といった形です。この呼び出しはlinear typeのborrow trackerによってrejectされます。poolが1引数として消費されている最中に`pool[2]`が別引数として借り出されるため、compose時に`UnreturnedBorrowError`として表面化します。
#
# workaroundとしてはconcrete modeに切り替えて、各スロットを個別に渡します(Section 4.2参照)。concrete modeなら4つの`pool[i]`は独立した`Qubit`引数(3つは制御、1つはtarget)になり、borrow trackerも満足します。
...
# - **一部の機能はモード固有です。** 複数の独立したpositional制御引数とscalar + `VectorView`混合はconcrete専用(Section 4)、単一poolの呼び出し形、`num_controls = <UInt式>`、subset selectorの`controlled_indices=`はsymbolic専用(Section 5)です。
#
# 実用的な判断ルールとしては、制御数がカーネルパラメータやそれを含む式の場合、特に「最後の1つ以外全部」の典型形`num_controls=n - 1`を含む場合は、迷わず*symbolic mode*を使ってください。制御数がリテラルで、制御対象として個別に名指ししたい特定のqubitがある場合は*concrete mode*を使います。
#
