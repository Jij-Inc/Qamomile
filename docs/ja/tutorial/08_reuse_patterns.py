...
# %% [markdown]
# ### opaque オラクルを含むqkernelのリソース推定
#
# `estimate_resources()`は、オラクル内部が未実装でもqkernel全体を解析できます。既知の回路部分は通常どおり集計され、未知の opaque 部分は`est.calls.calls_by_name` / `est.calls.queries_by_name`として追跡されます。
...
# %% [markdown]
# 次に、通常ゲートと複数の opaque オラクルを混在させたqkernelで確認します。
...
assert oracle_est_4.calls.queries_by_name == {"oracle": 10, "mixing": 4}

# %% [markdown]
# この例のように、オラクル内部が不明でも回路解析を進められます。既知部分は通常通りカウントされ、未知オラクル部分は`calls_by_name`（例: `{'oracle': rounds + 1, 'mixing': rounds}`）と`queries_by_name`（query complexityで重み付け）として追跡されます。
