...
# | **CUDA-Q** | サポート済み | GPUアクセラレーテッドシミュレーション。対応: forループ（展開）。未対応: 測定結果に依存する条件分岐、whileループ |
#
# > **注意**：すべての機能がすべての量子SDKで使えるわけではありません。例えば測定結果に依存する条件分岐（測定ビットに対する`if`）はQiskitでは対応済みですが、CUDA-Q（0.14.x）では未対応です。ミッドサーキット測定自体は可能ですが、その結果に基づく分岐はトランスパイル時に`EmitError`が発生します。コンパイル時定数の条件はすべてのバックエンドで静的に解決されます。
#
# ### CUDA-Q プラットフォームサポート
#
