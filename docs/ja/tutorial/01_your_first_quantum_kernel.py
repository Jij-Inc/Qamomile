...
# - **トランスパイル**: カーネルをバックエンド固有の実行可能形式にコンパイル。
...
# > バックエンドへのトランスパイル時にゲートが分解・最適化されることがあるため、
# > 実際に実行される回路は `draw()` の表示と異なる場合があります。
# > バックエンド固有の回路を確認するには `to_circuit()` を使ってください。
...
#
# 1. **トランスパイル**: カーネルをバックエンド実行可能な形式にコンパイル。
...
# バックエンドが実際に受け取る回路を確認できるので、デバッグに便利です。
...
#
# > **注意**: すべての機能がすべてのバックエンドで使えるわけではありません。
# > 例えば `if` 分岐は Qiskit では対応済みですが、他のバックエンドでは未対応の
# > 場合があります。未対応の場合はトランスパイル時にエラーが出ます。
# > 迷ったら開発時は Qiskit を使い、デプロイ時にバックエンドを切り替えてください。
#
# ## 次の章
#
