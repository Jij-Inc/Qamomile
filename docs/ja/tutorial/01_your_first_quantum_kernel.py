...
#
# > **注意**：ランタイム測定依存の制御フロー（`if bit:`、`if/else`、`while bit:`）はQiskitとCUDA-Qの両方でサポートされています。CUDA-Qでは、すべての回路が`@cudaq.kernel`デコレータ関数としてコンパイルされます。ランタイム制御フローを含まない静的回路は`cudaq.sample()` / `cudaq.observe()`経由で実行され、ランタイム測定依存の分岐を含む回路は`cudaq.run()`経由で実行されます。コンパイル時定数の条件はすべてのバックエンドで静的に解決されます。
#
# ### CUDA-Q プラットフォームサポート
#
