...
# ポイント：
#
# - **型アノテーションは必須**です。`theta: qmc.Float`はthetaが浮動小数点
#   パラメータであることを、戻り値の`qmc.Bit`はこの量子カーネルが
#   1つの古典ビットを出力することを示します。
# - **`qmc.qubit(name="q")`**は量子ビットハンドルを作成します。`name`は
#   回路図に表示されます。
# - **`q = qmc.ry(q, theta)`**はRYゲートを適用し、**`q`を再代入**します。
#   この再代入が重要です — 理由は後述します。
...
#   値を与える必要があります（チュートリアル02で詳しく扱います）。
#
# - **`bindings={"theta": math.pi / 4}`**（実行時）— パラメータに
#   具体的な値を設定します。
#   デフォルトのexecutorはローカルシミュレータですが、
#   カスタムexecutor（実機やクラウドサービス）を使うこともできます。
#
...
# - **`cx()`**（CNOTゲート）— 2量子ビットゲートで、**両方の**ハンドルを返す
...
#   `q[0]`、`q[1]`のようにインデックスでアクセスします。チュートリアル02では
#   `qubit_array(n)`でサイズをシンボリックにする方法を扱います。
# - **2量子ビットゲートは両方のハンドルを返す**: `q[0], q[1] = qmc.cx(q[0], q[1])`。
#   両辺を再代入する必要があります。
#
