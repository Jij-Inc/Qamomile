...
# ## Qamomile とは？
#
# Qamomile は量子プログラミング SDK です。Python で量子プログラムを記述し、記述したプログラムはサポートされている量子SDK（Qiskit、QuriParts、その他今後対応予定）で実行できます。**型アノテーション付きのシンボリック**なアプローチを採用しており、`@qkernel` デコレータを付けた Python 関数を書くと、Qamomile がそれをトレースし、分析・可視化・量子SDKへの変換可能な中間表現が作成できます。
#
...
# ```
#
# このチュートリアルでは、具体的な量子SDKとして Qiskit を使用します。QuriParts もサポートされており、トランスパイル可能な量子SDKは今後も増えていく予定です。
...
# ## 最初の量子カーネル：偏りのあるコイン
#
# **量子カーネル**とは、`@qmc.qkernel` デコレータを付けた Python 関数です。型付きハンドルとゲート操作を使って量子回路を記述します。
# > ハンドルとは、あるリソースやオブジェクトを間接的に参照するための「識別子」や「トークン」のことです。
#
# 最もシンプルな例を作りましょう：1 つの量子ビットを角度 `theta` で回転させ、測定します。`theta` の値に応じて、量子ビットは `0` または `1` に偏ります。偏りのあるコイントスのようなイメージです。
...
# ポイント：
#
# - **型アノテーションは必須**です。 `theta: qmc.Float` は theta が浮動小数点
#   パラメータであることを、戻り値の `qmc.Bit` はこの量子カーネルが
...
#
# > **注意**: `draw()` は Qamomile の IR レベルで回路を可視化します。対象の量子SDKへのトランスパイル時にゲートが分解・最適化されることがあるため、実際に実行される回路は `draw()` の表示と異なる場合があります。対象量子SDK固有の回路を確認するには `transpiler.to_circuit()` を使ってください。
...
# %% [markdown]
# この量子カーネルでは具体的な数値が返りますが、パラメータ付き量子カーネルではSymPy を用いた代数的なリソース推定も可能です —[チュートリアル 02](02_parameterized_kernels.ipynb) で詳しく扱います。

# %% [markdown]
# ## 実行パイプライン
#
# 実際に量子カーネルを実行してみましょう。3 ステップです：
#
...
# 3 つの概念を押さえておきましょう：
#
# - **`parameters=["theta"]`**（トランスパイル時）— 量子カーネル入力のうち、
#   トランスパイル後もスイープ可能なパラメータとして残すものを宣言します。
#   ここに列挙されていない入力は、実行時に `bindings` で
#   値を与える必要があります（チュートリアル 02 で詳しく扱います）。
#
# - **`bindings={"theta": math.pi / 4}`**（実行時）— パラメータに
#   具体的な値を設定します。
#   デフォルトの executor はローカルシミュレータですが、
#   カスタム executor（実機やクラウドサービス）を使うこともできます。
#
...
#
# 例えば、`[(0, 150), (1, 106)]` は、256 ショットのうち結果 `0` が 150 回、結果 `1` が 106 回出現したことを意味します。
...
# ## トランスパイル後の回路の確認
#
# `to_circuit()` はすべてのパラメータをバインドした状態でトランスパイルし、量子SDK固有の回路（例: Qiskit の `QuantumCircuit`）を返します。量子SDKの形式で回路を確認できるので、デバッグに便利です。
...
# ## アフィン型システム
#
# Qamomile の量子ハンドルは**アフィン型**です。ゲートがハンドルを消費したら、以降は返された新しいハンドルを**必ず**使う必要があります。
#
...
# ### なぜ線型ではなくアフィンなのか？
#
# 量子コンピューティングでは、一時的な量子ビットをシステムともつれたまま放置すると、他の量子ビットに予期しない影響を与える可能性があります。厳密には **線型型**（すべてのハンドルをちょうど 1 回使う）が最も安全ですが、Python で線型型を強制すると書きづらくなるため、Qamomile では**アフィン型**を採用しています：ハンドルは**最大 1 回**使用でき、ドロップ（使わずに捨てること）も許可されます。
#
...
# ## サポートされている量子SDK
#
# Qamomile は同じ `@qkernel` を異なる量子SDKにトランスパイルします。現在のサポート状況：
#
# | 量子SDK | ステータス | 備考 |
# |---------|--------|-------|
...
#
# > **注意**: すべての機能がすべての量子SDKで使えるわけではありません。例えば `if` 分岐は Qiskit では対応済みですが、他のSDKでは未対応の場合があります。未対応の場合はトランスパイル時にエラーが出ます。
#
# ## 次の章
#
