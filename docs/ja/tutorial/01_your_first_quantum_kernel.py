...
# | **CUDA-Q** | サポート済み | GPUアクセラレーテッドシミュレーション。対応: `c_if`（if-then のみ、else なし）、forループ（展開）。未対応: whileループ |
#
# > **注意**：すべての機能がすべての量子SDKで使えるわけではありません。例えば`if`/`else`分岐はQiskitでは対応済みですが、CUDA-Qでは`if`-then（`else`なし）のみ対応しています。未対応の場合はトランスパイル時にエラーが出ます。
#
# ### CUDA-Q プラットフォームサポート
#
