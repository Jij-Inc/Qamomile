...
#
# > **注意**：ランタイム測定依存の制御フロー（`if bit:`、`if/else`、`while bit:`）はQiskitとCUDA-Qの両方でサポートされています。CUDA-Qでは、これらの回路は自動的に`@cudaq.kernel`関数としてコンパイルされ、`cudaq.run()`経由で実行されます。ランタイム制御フローを含まない静的回路は、引き続きビルダーAPIと`cudaq.sample()`を使用します。コンパイル時定数の条件はすべてのバックエンドで静的に解決されます。
#
# ### CUDA-Q プラットフォームサポート
#
