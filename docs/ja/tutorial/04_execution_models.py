...
# # 実行モデル: `sample()` と `run()`
#
# Qamomile は、カーネルの戻り値に応じて2つの実行メソッドを提供します:
#
...
#
# この章では、両方のモードを説明し、期待値計算のための**オブザーバブル**を紹介します。
...
# ## 複数量子ビットの `sample()`
#
# チュートリアル01では単一の `Bit` をサンプリングしました。カーネルが複数のビットを返す場合、
...
# ## 期待値が必要な場合
#
# 個々の測定結果ではなく、量子オブザーバブルの**平均値**が必要な場合があります。
# これは以下のような場面で一般的です:
#
...
#
# このために、Qamomile は `expval()` と `run()` 実行メソッドを提供します。
...
# 理解すべき関連する2つの概念があります:
#
# 1. **`qmc.Observable`** — カーネルのシグネチャで使用される**ハンドル型**です。
#    `qmc.Float` が数値のプレースホルダーであるのと同様に、
#    トレーシング中のプレースホルダーです。
#
# 2. **`qamomile.observable` モジュール** — バインディングで渡す**具体的な**オブザーバブル値を
#    構築する場所です。例えば:
#
# ```python
# import qamomile.observable as qmo
...
# ```
#
# 次のように考えてください: `qmc.Observable` はカーネル内の**型アノテーション**であり、
# `qmo.Z(0)` はトランスパイル時にバインドする**値**です。
...
# ## `run()` による実行
#
# 期待値カーネルには、`sample()` の代わりに `run()` を使用します。
# オブザーバブルはトランスパイル時にバインドされ（測定回路に影響するため）、
# `theta` はスイープ可能なランタイムパラメータとして残ります。
...
# ## まとめ
#
# - `sample()` は測定ビットを返すカーネル用です — カウント付きの
#   測定結果の分布が得られます。
# - `run()` は `expval()` を通じて `Float` を返すカーネル用です — 単一の
#   期待値が得られます。
# - `qmc.Observable` はハンドル型であり、`qamomile.observable.Z(0)` などが
#   具体的な値です。オブザーバブルはトランスパイル時にバインドします。
# - ビット順序はビッグエンディアンです: 戻り値タプルの位置が量子ビットの順序に対応します。
#
# **次へ**: [古典フロー・パターン](05_classical_flow_patterns.ipynb) — `qmc.range` によるループ、
# `qmc.items` によるスパースデータ、条件分岐について。
