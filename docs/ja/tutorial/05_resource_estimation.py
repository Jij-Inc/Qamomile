...
# | `if`、`for`、`for_items`、`while` | 可能な場合はコンパイル時のconditionとloop boundをsymbolicに保ちます。パラメータ分岐のgate数と幅は`Piecewise`で表し、測定結果に基づく分岐は保守的な最大値を使います。symbolicなaliasや複数wireの境界により厳密なscheduleを構成できない場合、dependency depthは`conservative`のままになります。loopのworkは累積し、再利用できる幅はlivenessに従います。扱えないcarry recurrenceは明示的にエラーにするか、仮定として表示します。 |
# | `qmc.expval(...)` | 抽象的なexpectation query 1回とmeasurement layer 1段を記録します。observable grouping、basis rotation、shot数、executorのsampling policyはengine/executorに依存するため、架空のgate数を出さず、仮定を明示して`derivation=MODELED`、`quality=UNKNOWN`とします。 |
# | LCU block encoding | 直接、制御、inverse、descriptor経由、serialization後のいずれでも、signalレジスタとsystemレジスタの厳密な幅要件を維持します。 |

# %% [markdown]
# ### リソースcountの算出方法、quality、数学的なapproximation
#
# 三つの独立したfieldが別々の問いに答えます。`estimate.derivation`は、見えているIRと選択した推定器の分解規則から数えたか（`STRUCTURAL`）、宣言済みまたはfallbackのmodelを使ったか（`MODELED`）を表します。`estimate.quality`は、そのcountが`EXACT`、過小評価しない`CONSERVATIVE`、方向を判断できない`UNKNOWN`のいずれかを表します。`estimate.approximation`は、選択した回路について推定器が認識した数学的近似を記録します。ここで`EXACT`は「既知の近似を記録していない」という意味で、opaque boundaryの内側や外部helperがすでに低水準化した処理の数学的意味まで厳密だと証明するものではありません。そのため、`CONSERVATIVE`だから`APPROXIMATE`になるわけではなく、modelを使った推定が同時に`CONSERVATIVE`になる場合もあります。
...
#
# 構造を決める具体値は、可能な限り最初の`estimate_resources()`呼び出しで`inputs`として渡します。こうするとdepth式を構築する前に、dependency schedulerが配列の物理index、view、loopで引き継がれるwireの同一性を解決できます。後から`.substitute()`してもsymbolic推定の安全な評価と維持された要件の検証はできますが、すでに保守的に構築されたalias scheduleは再構築できません。そのため、すべてのsymbolがconcreteになってもdepthは`conservative`のままになる場合があります。
...
# %% [markdown]
# 仮定、derivation/qualityの情報、callの集計、trace nodeには、数値リソースと同じsymbolicな分岐guardが付きます。上の例のように`inputs`または`.substitute()`で分岐を選ぶと、実行されない側の警告やopaque callは消えます。説明treeが必要な場合だけ`trace=True`を指定します。既定では保持しないため、推定結果は小さいままです。`est.explain()`を使うと、再帰的な本体、primitive、opaqueの説明を表示できます。
...
# %% [markdown]
# `quality`が`conservative`なのは、途中測定に基づくclassical feed-forwardの分岐を安全側に数え、`CLEAN_ANCILLA_TOFFOLI`が保守的な多制御モデルであるためです。算出方法は`structural`のままであり、この上界自体が回路を数学的な近似にするわけではありません。ここで得られるのはalgorithmic circuitのリソースであり、特定deviceのnative gateへの分解、routing、誤り訂正、magic state生成などは含みません。
