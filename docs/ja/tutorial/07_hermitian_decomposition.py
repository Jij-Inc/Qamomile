...
# # エルミート行列から量子回路へ
#
# 量子アルゴリズムの多くは、密な$2^n \times 2^n$のnumpy配列として与えられた
# **エルミート行列**（ハミルトニアン）から出発し、その時間発展$e^{-iHt}$を量子
# コンピュータ上でシミュレーションしたいという状況からはじまります。定石は以下の
# 2ステップです。
#
...
#
# 分解の内部では**Fast Walsh-Hadamard Transform**を使い、NumPyだけで
# $O(n \cdot 4^n)$時間で走ります。Qiskit依存はありません。
...
# $$
#
# 横磁場は$h = 0.7$とします。ここはQamomile固有の要素は何もなく、`np.kron`で
# 普通の$4 \times 4$のnumpy配列を組むだけです。
...
# ## ラップして分解する
#
# `HermitianMatrix`は、構築時に配列を検証（2D、正方、2冪の次元、エルミート）し、
# `to_hamiltonian()`を公開する薄いラッパです。返される`Hamiltonian`は
# `qamomile.observable`のもので、`pauli_evolve`や`expval`、最適化ヘルパが
# 使うものと同じ型です。
#
# Qamomileの分解では、**qubit 0が行列の計算基底インデックスの最下位ビット**に
# 対応します。これはQiskitの内部順序と一致しています。
...
# %% [markdown]
# 2サイト横磁場Isingモデルに対しては、ちょうど3つの非零項が期待されます。
# 係数$-1$の$Z_0 Z_1$と、係数$-h$の単一qubit $X$が2つです。
...
# ## `@qkernel`での時間発展
#
# `pauli_evolve(q, H, t)`はqubitレジスタに$e^{-iHt}$を作用させます。`H`は
# `qmc.Observable`ハンドル型で宣言され、トランスパイル時にbindings経由で渡されます。
# `t`はbindingでもよいですし、スイープ可能なパラメータのまま残しても構いません。
#
# ここでは$\lvert 00\rangle$から出発し、qubit 0にHadamardで重ね合わせを
# 作ってから時間発展のステップを適用します。最後に測定をつけているのは、
# `transpile()` / `to_circuit()`に渡すエントリポイントのカーネルが古典出力を
# 持つ必要があるためです。
...
# %% [markdown]
# 3つのbindingをすべて固定してトランスパイルします。Qiskit回路は
# `transpiler.to_circuit()`から素のQuantumCircuitとして返ってきます。
...
# ## 厳密な指数関数との比較による検証
#
# 小さなエルミート行列であれば、numpyの固有値分解から直接$U = e^{-iMt}$を組み、
# 同じ初期状態に作用させたうえで、Qamomileが生成した回路が同じ最終状態を
# （グローバル位相を除いて）与えることを確認できます。
...
# %% [markdown]
# フィデリティは数値的に$1$と区別できません。Pauli分解からQamomileが組み立てた
# 回路は、直接の行列指数関数と同じユニタリを実現しています。
#
# ## まとめ
#
# - `HermitianMatrix`は密なエルミートnumpy配列を検証し、FWHTベースの分解で
#   Pauli和への変換を担います。
# - `to_hamiltonian()`は`qamomile.observable.Hamiltonian`を返します。これは
#   `pauli_evolve`や`expval`など、量子アルゴリズム一式が使うのと同じ型です。
# - 両者が揃うことで、エルミート行列から出発するアルゴリズム（ハミルトニアン
#   シミュレーション、VQE、block-encoding / LCUのエルミート側など）に対し、
#   `np.ndarray → Hamiltonian → circuit`という直接経路が得られます。
#
# **非自己共役な作用素**（たとえば数値流体シミュレーションに現れる移流
# stencilなど）のサポートは自然な次のステップで、非エルミート行列の適切な
# 統合ポイントが決まり次第、改めて扱う予定です。
