...
#
# 同じ単一量子ビットゲートをレジスタ全体の各量子ビットに適用する場合は、
# インデックスのループを書く代わりに、配列をそのままゲートに渡せます。
# これは `for i in qmc.range(n): q[i] = qmc.gate(q[i])` と同じ
# 明示ループの記法と等価で、内部的にも同一の IR ループに展開されるため、
# リソース見積もり、トランスパイル、可視化のいずれにおいても挙動は変わりません。
# 回転ゲートの場合は、同一のスカラ角度がすべての量子ビットに適用されます。
# 量子ビットごとに異なる角度を渡す場合は、引き続き明示ループで
# パラメータベクトルをインデックスする必要があります。
...
# %% [markdown]
# `rotation_layer`（上）と `rotation_layer_broadcast` は同じ回路を生成します。
# レジスタ全体に同一のゲートを並べる「層」を書くときはブロードキャスト記法が
# 慣用的な選択です。各量子ビットに異なる角度（例：`Vector[Float]` をインデックスする）を
# 渡したい場合や、レジスタの一部のみを操作する場合は、引き続き明示ループを使ってください。
