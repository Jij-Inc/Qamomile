...
# %% [markdown]
# 上の例から分かる通り、`BinaryExpr.coefficients`には辞書形式で変数の係数が格納されており、キーは変数のインデックスのタプル、値はその変数の係数となっています。例えば、上の例では`x_1`と`x_3`の線形項の係数がそれぞれ1であるため、`(1,)`と`(3,)`がキーとなり、値は`x_1`が1.0、`x_3`が2.0になっています。また、二次項である`2 * x_1 * x_3`の係数は3であるため、キーは`(1, 3)`となり、値は3.0になっています。
...
# `from_ising`関数は`ising`引数としてIsingの係数を表す辞書形式のデータを取ります。ただし、`from_ising`では`linear`と`quad`という引数を取ります。`linear`引数は線形項の係数を表す辞書形式のデータで、キーは変数のインデックス、値はその変数の係数となっています。`quad`引数は二次項の係数を表す辞書形式のデータで、キーは変数のインデックスのタプル、値はその変数の係数となっています。
