...
関連の深いチュートリアル：[はじめての量子カーネル](../tutorial/01_your_first_quantum_kernel)、[パラメータ付き量子カーネル](../tutorial/02_parameterized_kernels)、[実行モデル: sample() vs run()](../tutorial/04_execution_models)、[再利用パターン: QKernelの合成とコンポジットゲート](../tutorial/06_reuse_patterns)

## リソース推定

Qamomile v0.10.0ではシンボリックなリソース推定機能が提供されます。任意のqkernelに対して`estimate_resources()`を呼び出すと、qkernelを**実行することなく**量子ビット数やゲートの内訳を取得できます。これらは入力パラメータのシンボリック式として表現されるため、問題サイズに対するリソースのスケーリングの解析が容易です。具体的な値を代入して、特定の入力サイズに対する具体的な推定値を得ることも可能です。さらに、実装を持たないブラックボックスオラクルをリソースメタデータ付きのopaque callableとしてモデル化でき、これを用いたqkernelに対するリソースの推定も可能です。
...
関連の深いチュートリアル：[リソース推定](../tutorial/03_resource_estimation)、[再利用パターン: QKernelの合成とコンポジットゲート](../tutorial/06_reuse_patterns)
...
関連の深いチュートリアル：[実行モデル: sample() vs run()](../tutorial/04_execution_models)、[qBraidサポート - QBraidExecutor](../integration/qbraid_executor)
