...
# # Qamomile v0.10.0
#
# Qamomile v0.10.0は、量子プログラミング層をゼロから再構築したリリースです。これにより従来のスコープである量子最適化アルゴリズム開発のみならず、より一般的に任意の量子プログラム/アルゴリズムの開発ををカバーできるようになりました。v0.9.0からの変更点は非常に多岐にわたるため、差分の一覧ではなく、**v0.10.0で何ができるようになったか**をご紹介します。
#
...
# ## マルチ量子SDKトランスパイル
#
# `@qkernel`で回路を一度定義すれば、対応するサポート済みの量子SDKの形式へとトランスパイルが可能です。トランスパイルされたqkernelは対応する変換先の量子SDK毎の`ExecutableProgram`となりまず。Qamomile v0.10.0には各量子SDKにプリセットのExecutorが用意されているため，変換先の量子SDKのコードを書かずとも実行まで行うことができます。このプリセットのExecutorは原則シミュレータ実行を仮定しています。より細かい制御や実機へのアクセスのために、カスタムExecutorを作成することもできます。
#
