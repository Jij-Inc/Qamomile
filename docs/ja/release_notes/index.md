...
- [v0.12.3](v0_12_3) — Python風の`Vector`スライシング、Pauli-Hamiltonian同士の`commutator(a, b)`、`computational_basis_state`アルゴリズムヘルパー、IRのcanonical form / content hash、`Block.param_slots`マニフェスト、`Block`のJSON / msgpackシリアライゼーション、ベクトルスライシング / QeMCMC / 量子カーネル分類のチュートリアル新設
