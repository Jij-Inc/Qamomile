...
#
# 画像認識は、入力画像をクラスへ分類する機械学習タスクです。通常の全結合層だけで画像を処理すると、画素間の空間的な関係を直接利用しにくく、多数のパラメータが必要になります。畳み込みニューラルネットワーク (CNN) は、小さなフィルタを画像全体で共有しながら適用することで、エッジやテクスチャなどの局所的なパターンを特徴量として抽出します。このとき、フィルタが一度に参照する画像内の局所領域を局所受容野と呼びます。さらに、プーリング層によって空間方向の解像度を段階的に圧縮します。こうして得られた特徴量を分類器へ入力することで、画像のクラスを識別します [](https://www.nature.com/articles/nature14539)。
#
# ### 量子ニューラルネットワーク(QNN)
#
# 量子ニューラルネットワーク (QNN) は、入力された量子状態にパラメータ依存のユニタリ変換を順番に作用させる教師あり学習モデルとして捉えられます。二値分類では、読み出し用量子ビット上の Pauli オブザーバブルを測定し、その出力を予測値とします。古典最適化アルゴリズムは、この予測値が訓練ラベルへ近づくようにユニタリ変換のパラメータを更新します。古典サンプルは最初に量子状態へ符号化する必要がありますが、同じ枠組みで量子入力状態に直接付与されたラベルを学習することもできます [](https://arxiv.org/abs/1802.06002)。
#
# 量子畳み込みニューラルネットワーク (QCNN) [](https://www.nature.com/articles/s41567-019-0648-8) は、CNNの局所受容野と重み共有の考え方を量子回路へ取り入れた画像向けのQNNです。小さな画像領域を量子状態へ符号化し、同じパラメータ付き量子回路を量子フィルタとして繰り返し適用して、測定された期待値から特徴量マップを構成します。こうした量子畳み込み層を古典層と組み合わせることで、現在の量子ハードウェアでも扱えるハイブリッドな画像分類器を構成できます [](https://iopscience.iop.org/article/10.1088/2632-2153/ad2aef)。
...
#
# 量子デバイスでは、期待値を有限回の測定結果から推定します。そのため学習には、推定値の平均が真の勾配と一致する勾配の不偏推定量が必要になります。生成子$G$の固有値が$\pm 1$である$e^{-i\theta G/2}$形式のゲートに対して、**パラメータシフトルール** [](https://doi.org/10.1103/PhysRevA.98.032309) [](https://doi.org/10.1103/PhysRevA.99.032331) は、パラメータを正負にずらした2つの期待値から勾配を求める厳密な関係式を与えます：
#
# $$
