...
# # 量子特異値変換
#
# 量子特異値変換（QSVT）は、より大きなユニタリに埋め込んだ行列の特異値へ多項式を適用する手法です。本記事では、非エルミートな$2\times2$行列のブロックエンコーディングを構築し、`qmc.qsvt`で2次のチェビシェフ多項式を適用して、変換後の行列と論理リソース推定を検証します。
...
# ### 量子信号処理（QSP）
#
# 量子信号処理は、1量子ビットのユニタリに符号化されたスカラー$x\in[-1,1]$を多項式応答へ変換します。代表的な規約の一つでは、信号ユニタリを次のように定義します。
#
# $$
# W(x)=e^{i\arccos(x)X}
# =\begin{pmatrix}
...
#
# QSPの設計では、まず目標の関数をこの条件を満たす多項式で近似し、対応する位相列を古典計算で合成します。次数$d$の変換は信号ユニタリを$d$回使うため、多項式の次数がクエリ計算量を決めます。この構成は、[Optimal Hamiltonian Simulation by Quantum Signal Processing](https://doi.org/10.1103/PhysRevLett.118.010501)でハミルトニアンシミュレーションに導入されました。
#
# QSVTは、このスカラー多項式変換をブロックエンコーディングされた行列の各特異値部分空間へ拡張します。この枠組みは、[Quantum singular value transformation and beyond](https://doi.org/10.1145/3313276.3316366)で展開されています。
...
#
# - **スパースアクセスオラクル**では、スパース行列の非ゼロ要素について、その位置と値を重ね合わせのまま取得します。これらのオラクルを組み合わせると、正規化されたスパース行列を射影ブロックに持つユニタリを構成できます。
# - **ユニタリの線形結合（LCU）**では、$A=\sum_j c_j U_j$を効率的に実装できるユニタリの重み付き和として表し、補助レジスタの振幅に$c_j$を反映して$U_j$を選択します。このレジスタを射影すると、$A/\sum_j|c_j|$が得られます。
#
# このほか、状態準備手続きや量子読み出し専用メモリ（QROM）に格納されたデータからもブロックエンコーディングを構成できます。これらの構成法は、[Quantum singular value transformation and beyond](https://doi.org/10.1145/3313276.3316366)にまとめられています。
...
# ### 問い合わせ計算量
#
# [GilyénらによるQSVTの回路構成](https://arxiv.org/abs/1806.01838)（3.2節、図1）では、$d$次の変換に$U$と$U^\dagger$を合計$d$回、射影位相回転を$d+1$回用います。したがって、ブロックエンコーディングに対する問い合わせ計算量は$\Theta(d)$です。必要な次数$d$は、目標関数、近似区間、許容誤差、正規化係数$\alpha$によって決まります。
#
