...
# # QURI Parts Support
#
# This page introduces Qamomile's [QURI Parts](https://quri-parts.qunasys.com/) backend and walks through the end-to-end flow on a concrete optimization problem: a small MaxCut instance, modelled as Ising via `BinaryModel.from_ising`, compiled to a QAOA ansatz with `QAOAConverter`, and executed through `QuriPartsTranspiler` / `QuriPartsExecutor`. `QuriPartsExecutor` defaults to [Qulacs](https://docs.qulacs.org/), a fast C++ state-vector simulator, so the examples below run on a local CPU without any extra configuration.
...
#
# `QAOAConverter` consumes a `BinaryModel` (or OMMX instance), exposes a Pauli-Z cost Hamiltonian via `get_cost_hamiltonian()`, and emits a measurement-ending QAOA sampling kernel via `transpile(transpiler, p=p)`. Passing `QuriPartsTranspiler()` as the transpiler is a one-liner, since the converter plugs into any Qamomile-supported backend.
...
# %% [markdown]
# Inspect the resulting QURI Parts circuit to confirm it is a linear-mapped parametric circuit and that the $2p$ QAOA angles (`gammas[0..p-1]`, `betas[0..p-1]`) survive as named runtime parameters:
...
# %% [markdown]
# The Python type is QURI Parts' linear-mapped parametric circuit (named `LinearMappedParametricQuantumCircuit` at runtime, aliased as `LinearMappedUnboundParametricQuantumCircuit` in the public API). Each runtime parameter slot stays unbound until execution time, so binding `gammas` / `betas` is a cheap numeric update on the QURI Parts side rather than a circuit rebuild. The problem structure (Ising coefficients, qubit count, depth) was folded away at compile time, and only the variational angles remain.

# %% [markdown]
# ## Sampling QAOA with `QuriPartsExecutor`
#
# `executable.sample(executor, bindings=..., shots=...)` returns a `SampleJob`. Calling `.result()` gives a `SampleResult`, which the converter decodes into a `BinarySampleSet` already in the spin domain (+1 / -1), so cut edges can be counted directly without further conversion. `QuriPartsExecutor()` runs against the Qulacs state-vector simulator by default.
...
#
# A typical QAOA optimization loop reuses the same `executable` across many `(gammas, betas)` vectors: one `converter.transpile()`, many `executable.sample()` calls. Below we wrap the sample-and-decode call into a SciPy `minimize` driver. The classical optimizer updates `(gammas, betas)` to minimize the mean sampled Ising energy, and every iteration reuses the same `executable` and `QuriPartsExecutor`.
...
#
# `QuriPartsExecutor.estimate_expectation(circuit, hamiltonian, param_values)` is the QURI Parts-native expectation-value entry point. It silently dispatches between two different QURI Parts estimators depending on the **state of the circuit it receives**:
#
# - **Unbound parametric circuit**: the linear-mapped circuit produced by `transpile()` still has free parameters. QURI Parts' `apply_circuit` wraps it as a `ParametricCircuitQuantumState`, and the executor routes the call through QURI Parts' **parametric estimator**, passing `param_values` to bind the parameters at evaluation time.
...
#
# Knowing which path fires matters because the two estimators have different cost profiles. The parametric estimator is the right tool inside a tight optimization loop, because it avoids a per-iteration circuit copy. The non-parametric estimator avoids parametric bookkeeping when the parameters are already concrete.
#
# QURI Parts treats `measure` as a no-op at the circuit level, so the parametric circuit emitted by `converter.transpile()` represents exactly the QAOA state preparation $|\boldsymbol{\gamma}, \boldsymbol{\beta}\rangle$. We can feed it straight to `estimate_expectation` together with the cost Hamiltonian to compute $\langle H_C \rangle$ analytically, with no sampling noise. Inside a QAOA optimizer you can therefore keep the same circuit and swap `executable.sample()` + decode for `executor.estimate(circuit, hamiltonian, params=...)`.
#
# To exercise both estimator paths directly, build the QURI Parts Operator from the converter's cost Hamiltonian and call `estimate_expectation` on each circuit variant:
...
# %% [markdown]
# Both paths agree to numerical precision because they evaluate the same QAOA state against the same Ising cost Hamiltonian. The resulting noise-free expectation value at the optimized parameters also lines up with the optimized sample-mean energy printed earlier. The dispatch is invisible to callers who stay inside Qamomile's `executor.estimate()` facade. Reach for `estimate_expectation` directly only when you have a reason to manage the QURI Parts circuit yourself.
#
# `executor.estimate(circuit, hamiltonian, params=...)` is the higher-level variant that accepts a `qamomile.observable.Hamiltonian` directly and auto-converts it before delegating to `estimate_expectation`:
...
# ```
#
# The custom executor is a drop-in replacement everywhere `executor` appeared above. Swapping the sampler does not require re-transpiling the kernel: the executable carries the circuit and the executor carries the simulation backend, so the two are independent.

# %% [markdown]
# ## Summary
#
# - `BinaryModel.from_ising` + `QAOAConverter` is the spin-domain recipe for MaxCut: hand the converter the Ising coefficients, call `converter.transpile(QuriPartsTranspiler(), p=p)`, and the QAOA sampling circuit is ready as an `ExecutableProgram` wrapping a linear-mapped parametric QURI Parts circuit.
...
# - `estimate_expectation` dispatches between QURI Parts' parametric and non-parametric estimators depending on whether the input circuit still has free parameters; `executor.estimate()` hides the dispatch, but you can also invoke it directly when integrating outside the Qamomile facade.
# - Custom samplers and estimators slot in through the executor without re-transpiling the kernel.
