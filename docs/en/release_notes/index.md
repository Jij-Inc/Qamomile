...
- [v0.12.3](v0_12_3) — Python-style `Vector` slicing, `commutator(a, b)` for Pauli-Hamiltonians, `computational_basis_state` algorithm helper, IR canonical form / content hash, `Block.param_slots` manifest, JSON / msgpack `Block` serialization, new Vector Slicing / QeMCMC / Quantum Kernel Classification tutorials
