...
`qmc.pauli_evolve(q, H, gamma)` lowers `exp(-i * gamma * H)` as a single-step first-order Trotter product: each Hamiltonian term `coeff * P` becomes one `exp(-i * gamma * coeff * P)` rotation, applied once in Hamiltonian iteration order. When the terms of `H` do not all commute this is an `O(gamma^2)` approximation of the exact propagator, not the exact evolution. Every emit backend behaves the same way — the shared `h` / `rz` + CX-ladder gadget on the Qiskit manual path and QURI Parts, the native `exp_pauli` on CUDA-Q — so the approximation is a property of the IR-level decomposition, not of any one backend.
