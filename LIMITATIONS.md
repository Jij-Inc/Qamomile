...
The runtime output and ordinary qkernel-call paths now preserve `TupleValue` / `DictValue` metadata closely enough for direct tuple outputs, tuple element outputs, post-classical tuple element expressions, structural input/output serialization, and caller-side indexing / `qmc.items()` on a sub-qkernel's returned tuple or dict. Some less-common compiler paths still assume block outputs are plain scalar `Value` objects or only rewrite a structural value's top-level UUID, so nested structural values can keep stale element references or fail before lowering.

**When it bites**: the most likely remaining shapes are (1) `InlinePass` rewriting a helper whose returned `TupleValue` / `DictValue` contains elements that must be substituted into caller-side values, especially inside nested blocks or composite-gate implementation blocks; (2) `CompileTimeIfLoweringPass` lowering a block whose `output_values` contain a `TupleValue` / `DictValue` wrapping a Phi result, because only substituted plain `Value` results are accepted; (3) `qmc.if_else(...)` branches that return `qmc.Tuple[...]` or `qmc.Dict[...]` handles, because the frontend Phi merge is still scalar-`Value` oriented; and (4) nested tuple runtime bindings such as `pair: qmc.Tuple[qmc.Tuple[qmc.UInt, qmc.UInt], qmc.UInt]` followed by `return pair[0][1]`, because execution-context alias seeding currently descends only to direct tuple elements.

**Why this trade-off was chosen**: the current fix focused on the user-visible paths that were already reproducible in normal transpilation and execution: post-process sample aggregation, scalar/tuple measured outputs, direct tuple element runtime bindings, structural input/output serialization, and top-level sub-qkernel structural return wrapping. The remaining cases are broader pipeline contracts: deep structural substitution in `InlinePass`, structural Phi substitution in compile-time control-flow lowering, structural branch merging in the frontend, and recursive runtime binding alias generation. Each needs a shared recursive `ValueLike` substitution/wrapping model rather than another one-off scalar fallback.

**Workaround**: avoid returning structural `Tuple` / `Dict` values directly from `qmc.if_else` branches, avoid relying on nested tuple runtime bindings as public inputs, and flatten nested tuple inputs into separate scalar or direct tuple arguments when the elements must be used in post-classical expressions. For helper qkernels, returning direct scalar elements that the caller needs is safer than returning a newly assembled structural container that must later be inlined and re-indexed.

**Future fix**: add a single recursive `ValueLike` substitution utility used by `InlinePass`, `CompileTimeIfLoweringPass`, and any operation-owned nested-block rewriting, then teach the frontend Phi merge to rebuild structural handles from merged structural `ValueLike` results. Runtime binding alias seeding should recurse through nested `TupleValue` elements while preserving the current top-level-name collision protections.
