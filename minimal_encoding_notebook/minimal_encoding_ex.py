...
from minimal_encoding_vrp import *
import warnings
...
def main():
    ...
    # n_eval = 100
    # estimator = Estimator()
    # cost_function = init_cost_function(nc, nr, na, circuit, A, progress_history)
    ...
    points = ['栃木県', '群馬県', '埼玉県', '北海道']

    geo_data, instance_data = tsp_instance_data(points, nc)
    ...
    qubo, const = pubo_builder.get_qubo_dict(multipliers = {'one-city': 100, 'one-time': 100})
    ...
    # print(type(result))
    # # decode a result to JijModeling sample set
    # tsp_solution(result, pubo_builder, compiled_model, geo_data, instance_data)
    ...
    for i in range(10):
        ...
        H = define_pauli_op(nr)
        Ha = define_pauli_op(nr, ancilla=True)
        optimizer = COBYLA(maxiter=n_eval, tol=0.001, args=(circuit, estimator, cost_function, H, Ha, progress_history),)
    ...
    print(dir(result))
    print(type(result))
    tsp_solution(sample, pubo_builder, compiled_model, geo_data, instance_data)
    print(f'prgress history: {progress_history}')
    plt.plot(progress_history)
    plt.xlabel('number of iteration')
    plt.ylabel('value of cost function')
...
def tsp_instance_data(points:list[str], nc:int)->tuple[dict, dict]:
    '''
...
def tsp_solution(result:oj.sampler.response.Response, 
    ...
    print("tour: ", tour)
    to = TwoOpt(instance_data['d'])
    tour = to.local_search(tour)
