...
class QRAC31Builder:
    ...
    def get_hamiltonian(
        ...
        qubo, constant = self.pubo_builder.get_qubo_dict(multipliers=multipliers, detail_parameters=detail_parameter)
        ising = qubo_to_ising(qubo)
        _, color_group = greedy_graph_coloring(ising.quad.keys(), max_color_group_size=3)
        qrac_hamiltonian, offset, encoding = qrac31_encode_ising(ising, color_group)
        return qrac_hamiltonian, offset + constant, QRACEncodingCache(color_group, encoding)

    def decode_from_binary_values(self, binary_list: typ.Iterable[list[int]]) -> jm.SampleSet:
        binary_results = [{i: value for i, value in enumerate(binary)} for binary in binary_list]
        binary_encoder = self.pubo_builder.binary_encoder
        decoded: jm.SampleSet = jmt.core.pubo.binary_decode.decode_from_dict_binary_result(binary_results, binary_encoder, self.compiled_instance)
...
def transpile_to_qrac31_hamiltonian(compiled_instance, normalize=True) -> QRAC31Builder:
    pubo_builder = jmt.core.pubo.transpile_to_pubo(compiled_instance, normalize=normalize)
